...
const networkRetryDelayMs = 10 * 1000;
...
async function downloadRelease(version: string): Promise<string> {
  ...
  try {
    downloadPath = await downloadToolWithRetries(downloadUrl);
  } catch (err) {
    throw formatDownloadError(version, err);
  }
...
export async function downloadToolWithRetries(
  ...
  return networkOperationWithRetries("Download", () =>
    tc.downloadTool(downloadUrl),
  );
}

async function networkOperationWithRetries<T>(
  description: string,
  operation: () => Promise<T>,
): Promise<T> {
  for (let retry = 0; ; retry++) {
    try {
      ...
      if (retry >= networkRetries) {
        throw err;
      }

      core.debug(`${description} attempt failed: ${err}`);
      process.stdout.write(
        `${description} failed. Waiting ${
          networkRetryDelayMs / 1000
        }s before retry ${retry + 1}/${networkRetries}.` + os.EOL,
      );
...
function formatDownloadError(version: string, err: unknown): Error {
  ...
  return new Error(`Failed to download version ${version}: ${err}`);
}
...
async function fetchVersions(
  ...
  const httpClient = new httpm.HttpClient("setup-protoc");
  const headers =
  ...
  for (let pageNum = 1, morePages = true; morePages; pageNum++) {
    const releasesUrl =
      ...
      pageNum;
    const p = await networkOperationWithRetries("GitHub releases request", () =>
      httpClient.getJson<IProtocRelease[]>(releasesUrl, headers),
    );
